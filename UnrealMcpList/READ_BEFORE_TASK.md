...
- 한 작업 턴 안에서는 하위 작업 하나의 완료·실패·커밋 뒤 즉시 다음 첫 번째 `TODO`를 계속 처리한다. 진행 보고는 `commentary`를 우선 사용한다.
- 전체 목록이 끝나기 전에 작업 턴이 종료되면, 응답은 현재 처리 결과와 다음 첫 번째 `TODO`만 기록하는 진행 보고다. 전체 작업이 완료된 것처럼 표현하지 않는다.
...
- 한 작업 턴 안에서는 사용자 확인을 기다리지 않고 이 절차에 따라 다음 작업을 계속 처리한다. 이후 메시지에서 재개할 때도 현재 첫 번째 `TODO`부터 다시 적용한다.
