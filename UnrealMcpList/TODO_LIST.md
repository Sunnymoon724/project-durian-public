...
- TODO 재개 대상: `2-3`~`2-6`, `2-8`~`2-9`, `3-1`, `3-3`~`3-9` — 아래 재시도 메모와 검증 한계를 참고해 보강 후 진행한다.
- 다음 작업: `2-3` 얼음 생성 링 머티리얼 보강
...
- 실패 사유 (2026-09-19): Unreal MCP의 부동 PIE에서 `Q`로 자력 모드 진입과 우클릭 취소 뒤 정리를 화면으로 확인했다. 그러나 현재 TestMap 전방에는 자력 후보 액터가 없어 조준·잡기·연결선 및 벽 가림을 검증할 수 없었다. Slate 기반 입력은 카메라 회전을 제공하지 않고 컴퓨터 제어 표면에도 Unreal 창이 노출되지 않아 대상 위치 탐색을 진행할 수 없었다.
...
- 실패 사유 (2026-09-19): 실제 에셋은 `/Game/Resources/VFX/Cryonis/Materials/M_IceSpawnRing`에 있으며, C++ 참조와 일치시켰다. `OuterRadius`·`InnerRadius`·`RingOpacity` 파라미터를 0.5초 동안 확장·감쇠하도록 보강하고 머티리얼 컴파일·저장까지 성공했으나, 열려 있는 에디터가 점유한 빌드 작업이 종료되지 않아 C++ 모듈을 재빌드할 수 없었다. 따라서 PIE에서 깊이 가림을 검증할 수 없었다.
...
- 실패 사유 (2026-09-19): `MI_IceSpawnRing`을 생성하고 `RingColor`·`RingWidth`·`RingSpeed` 오버라이드를 설정했으며, C++ 참조도 인스턴스로 변경했다. 부모 머티리얼 컴파일·저장은 성공했지만, 에디터가 점유한 빌드 작업 때문에 C++ 모듈 재빌드와 PIE 인스턴스 동작 검증을 수행할 수 없었다.
